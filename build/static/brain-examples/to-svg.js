...
const recurrentJSONTypes = [ 'RNN', 'LSTM', 'GRU', 'RNNTimeStep', 'LSTMTimeStep', 'GRUTimeStep', 'Recurrent' ];
			...
			if (column === 0) {
				...
				for (
					...
					svg += drawBackwardConnections(
						Object.assign({ pixelX, pixelY, row, column, lineY, previousConnectionIndex }, options)
					);
				}
			}
		}
...
function toSVG(net, options) {
	...
	if (net instanceof NeuralNetwork || net instanceof RNN || net instanceof RNNTimeStep) {
		...
		sizes = [ inputLayer.height ].concat(hiddenLayers.map((l) => l.height)).concat([ outputLayer.height ]);
